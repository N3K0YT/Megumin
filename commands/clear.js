exports.run = (client, msg, args) => {
	...
	if (!args[0]) return msg.reply('please specify how many messages to delete.').then(yeet => {yeet.delete(5000)})
	...
	msg.channel.bulkDelete(args[0]).then(yoink => {
		msg.channel.send(`Cleared ${args[0]} messages`).then(yee => {yee.delete(10000)})
})
}